...
class Kormo_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'kormo_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'kormo-control-admin-css', get_template_directory_uri() . '/inc/css/kormo-customizer.css',array());
			wp_enqueue_script( 'kormo-control-admin-js', get_template_directory_uri() . '/inc/js/kormo-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function kormo_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new Kormo_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function kormo_data_load() {

		$data = apply_filters( 'kormo_customizer_data', array());
...
add_action('init','Kormo_customizer::kormo_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class Kormo_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','kormo'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','kormo'); ?></span>  
